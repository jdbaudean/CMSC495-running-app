...
public class MySQLiteHelper extends SQLiteOpenHelper {
    ...
    private static final String DATABASE_NAME = "runlog.db";
    private static final int DATABASE_VERSION = 1;

    // Database creation sql statement
    private static final String DATABASE_CREATE = "create table "
            + TABLE_RUNLOG + "(" + COLUMN_ID
            + " integer primary key autoincrement, " + COLUMN_DISTANCE
            + " real, " + COLUMN_DATE + " integer, " + COLUMN_DURATION + " integer );";
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_RUNLOG);
        onCreate(db);
    }
}
